...
import { getTopLosers } from '@/utils/priceUtils';
...
interface LoserItem extends PriceChange {
  rank: number;
  fruitName: string;
  marketName: string;
}
...
export default function TopLosers({ priceData, fruits, markets, limit = 10 }: TopLosersProps) {
  const theme = useTheme();

  const losers = useMemo(() => {
    const topList = getTopLosers(priceData, limit);
  ...
  const maxChange = useMemo(() => {
    ...
    return Math.max(...losers.map((l) => Math.abs(l.changePercent)));
  }, [losers]);
      ...
      {losers.map((item, index) => {
        ...
        return (
          <MotionBox
            ...
            whileHover={{ borderColor: theme.colors.price.down, boxShadow: '0 1px 2px 0 rgba(0, 0, 0, 0.05)' }}
            p={3}
            bg="white"
            borderRadius="md"
            border="1px solid"
            ...
            <Flex align="center" ml="40px">
              ...
              >
                <MotionBox
                  ...
                  style={{
                    background: `linear-gradient(90deg, ${theme.colors.price.down}99, ${theme.colors.price.down})`,
                  }}
                />
              </Box>
              <Text fontSize="sm" fontWeight="600" color={theme.colors.price.down} flexShrink={0}>
